...
clean_inspecs[, "merge2"] = ifelse(!is.na(clean_inspecs$mineid.y) & !is.na(clean_inspecs$mineid.x), 3, 0)
...
for (i in 1:length(common_varstbs)) {
  ...
  clean_inspecs[, mstr_copy] = ifelse(clean_inspecs[, "merge2"] == 3 & is.na(clean_inspecs[, mstr_copy]), clean_inspecs[, usng_copy], clean_inspecs[, mstr_copy])
  clean_inspecs[, mstr_copy] = ifelse(clean_inspecs[, "merge2"] == 2, clean_inspecs[, usng_copy], clean_inspecs[, mstr_copy])
}

insp_conflcts = sum(clean_inspecs[, "merge2"] == 3 & !is.na(clean_inspecs[, "sumtotal_insp_hours.x"]) & (clean_inspecs[, "sumtotal_insp_hours.x"] != clean_inspecs[, "sumtotal_insp_hours.y"]))
on_site_conflcts = sum(clean_inspecs[, "merge2"] == 3 & !is.na(clean_inspecs[, "sumtotal_on_site_hours.x"]) & (clean_inspecs[, "sumtotal_on_site_hours.x"] != clean_inspecs[, "sumtotal_on_site_hours.y"]))
